---
layout: single
permalink: /band
title: "Le groupe"
---


<div class="image-texte">
    <div class="image">
        <img src="/assets/images/press/CoeurDeLicorne1.jpg"/>
    </div>
    <div class="texte">
    <div>
    <p>
       Coeur de Licorne est un groupe familial composé de Papa Boudin et ses deux enfants, la Star et le professeur Prout. On a aussi Fleur St James qui nous filme et Fidel Castra qui nous aide à mixer les chansons.<br/> <br/>
       On veut pousser les enfants à se rebeller contre l'opression parentale, au moins le temps d'un concert, grâce au pouvoir du Roquénerol. Y'a un secret derrière le nom du groupe, mais faut venir aux concerts pour le découvrir !
    </p>
    </div>
    </div>
</div>


<div class="image-texte odd">
    <div class="image">
        <img src="/assets/images/songs/roquenerol.png"/>
    </div>
    <div class="texte"> <div>
    <p>
       <b>Roquénerol</b>, c'est justement le nom de notre CD ! Dessus y'a 6 compos mais 13 pistes : le reste c'est des fausses pubs, des histoires du soir et des blagues de mauvais goût.<br/> <br/>
       On fait surtout du rock mais aussi du reggae, du folk... On essaye de montrer aux enfants tout plein de techniques de guitare (bottleneck, pédale wah-wah, e-bow, looper...).
    </p>
    </div>
    </div>
    
</div>

<div class="image-texte">
    <div class="image">
        <img src="/assets/images/press/CoeurDeLicorne3.jpg"/>
    </div>
    <div class="texte"><div>
    <p>
     <b>En concert</b> c'est un peu n'importe quoi. La moitié du temps la Star vient même pas (elle ne se déplace que pour les Zeniths), du coup Papa Boudin a embauché Patoufette, une marionnette éfrontée qui pionce la moitié du temps. <br/> <br/>
     Heureusement le professeur Prout nous souve la mise avec sa démonstration de la Fart Machine, le fruit de longues années de recherches présentées en exclusivité mondiale.
    </p></div>
    </div>
    
</div>

<div class="image-texte odd">
    <div class="image">
        <img src="/assets/images/press/CoeurDeLicorne4.jpg"/>
    </div>
    <div class="texte">
      <div>
          <p>
          <b>Pourquoi on fait ça ?</b> Déjà parce qu'on adore faire des blagues nulles en familles. Et aussi parce que tout autour de nous on voit plein d'enfants qu'on force à faire plein de trucs, et qu'on n'écoute pas beaucoup. <br/> <br/>
          On aimerait bien qu'un jour on considère les enfants comme un classe sociale à part entière, et qu'on arrête de leur interdire l'accès à l'espace public ou à leurs droits fondamentaux.
          </p>
      </div>
    </div>
  </div>